...
static int callback_get_voltage(const struct _u_request * request, struct _u_response * response, void * _config) {
    ...
    while (TRUE) {
        ...
        if (current_timestamp - req_timestamp > RESP_TIMEOUT) {
            ulfius_set_string_body_response(response, 408, "Can response timeout\n");
...
static int callback_set_voltage(const struct _u_request * request, struct _u_response * response, void * _config) {
    ...
    json_t * j_voltage = json_object_get(json_body, "voltage_dac");
    if (j_voltage == NULL){
    ...
    if (json_is_integer(j_voltage)){
        voltage_dac = (unsigned16)json_integer_value(j_voltage);
    } else if (json_is_string(j_voltage)){
        voltage_dac = (unsigned16)strtol(json_string_value(j_voltage), NULL, 0);
    } else {
