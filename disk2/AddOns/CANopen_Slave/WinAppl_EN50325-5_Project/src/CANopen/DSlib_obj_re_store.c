...
#define CAN_NOF_RESTORE_SUBINDS				6	// Save also nodeid and bitrate
...
#define CAN_SUBIND_RESTORE_NODEID			5
#define CAN_SUBIND_RESTORE_BITRATE			6

#define CAN_RESTORE_DATA_SIZE_MAX			8	// Max size of the saved object (bytes)
...
static int32 get_object_size_words_16(canindex index, cansubind subind)
{
	...
	size = server_get_object_size(index, subind);
...
static int16 can_process_store_application(void)	// 3.0.3 some changes
{
	unsigned16 cob;
	int16 fnr;
...
static int16 write_index_store(cansubind subind)	// 3.0.4 some changes
{
	int16 fnr;
	unsigned16 lm;
	canbyte data;
	...
	if (subind == CAN_SUBIND_RESTORE_ALL) {
		...
		return put_flash_flag(PAGE_FLAG_RESMASK_ADDR, CAN_MASK_LOAD_COMM_1 | CAN_MASK_LOAD_COMM_2 | CAN_MASK_LOAD_APPL);
        ...
        if (server_read_object_dictionary(INDEX_EEPROM_CAN_NODEID, 0, &data) == CAN_RETOK) {
          return put_flash_flag(PAGE_FLAG_NODEID_ADDR, (unsigned16)data | MASK_FLAG_VALUE_SET);		// 3.0.2 placed here
        }
	} else if (subind == CAN_SUBIND_RESTORE_BITRATE) {
        if (server_read_object_dictionary(INDEX_EEPROM_CAN_BITRATE, 0, &data) == CAN_RETOK) {
          return put_flash_flag(PAGE_FLAG_BITRATE_ADDR, (unsigned16)data | MASK_FLAG_VALUE_SET);	// 3.0.2 placed here
        }
	}
	return CAN_ERRET_RE_STORE;
}
...
int16 get_flash_nodeid(void)    // 3.0.4 some changes
{
  int16 fnr;

  fnr = get_flash_flag_data(PAGE_FLAG_NODEID_ADDR);
  if (fnr < 0) return fnr;
  return (fnr & MASK_FLAG_VALUE_RETURN);
}

int16 get_flash_bitrate_index(void)    // 3.0.4 some changes
{
  int16 fnr;

  fnr = get_flash_flag_data(PAGE_FLAG_BITRATE_ADDR);
  if (fnr < 0) return fnr;
  return (fnr & MASK_FLAG_VALUE_RETURN);
}
...
void erase_flash_pages(void)		// 3.0.1 init as flash (0xFF)
{
	unsigned16 cnt;
...
void can_init_re_store(void)
{
	...
	stc_2[1].index = 0x1014;
	stc_2[2].index = 0x1400;
	stc_2[2].submask01 = 0x01;
	stc_2[3].index = 0x1401;
	stc_2[3].submask01 = 0x01;
	stc_2[4].index = 0x1402;
	stc_2[4].submask01 = 0x01;
	stc_2[5].index = 0x1403;
	stc_2[5].submask01 = 0x01;
	stc_2[6].index = 0x1800;
	stc_2[6].submask01 = 0x01;
	stc_2[7].index = 0x1801;
	stc_2[7].submask01 = 0x01;
	stc_2[8].index = 0x1802;
	stc_2[8].submask01 = 0x01;
	stc_2[9].index = 0x1803;
	stc_2[9].submask01 = 0x01;

  can_init_re_store_application();
}
